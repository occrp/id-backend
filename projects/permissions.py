...
class CanCreateListStoryTranslation(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if request.method in permissions.SAFE_METHODS:
            ...
            try:
                obj = StoryVersion.objects.get(id=view.kwargs['pk'])
            except Exception as e:
                return False

            return request.user.has_perm('story_translation.can_create_story_translation',
                                         obj)
...
class CanAlterDeleteStoryTranslation(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in permissions.SAFE_METHODS:
            ...
            # true here
            return True
            # return request.user.has_perm('story_translation.can_view_story_translation',
            #                              obj)
        else:
            return request.user.has_perm('story_translation.can_alter_or_delete_story_translation',
                                         obj)
